...
The 32 automated tests verify both complete role flows; explicit login after signup; final Dashboard/Profile routing; the six-link navigation and active state; exact role, institution, draft, publish, task and roadmap requests; shared roadmap/To-Do progress; institution history and engagement counts; Results restoration after a fresh login; editable AI fallback behavior; roadmap cache/generation/errors/retries; role guards; corrected homepage claims; and all earlier recommendation, validation, refresh and password-reset behavior.
