...
export function uploadStudentProfilePicture(
  userId: number,
  file: File,
) {
  ...
  return request<ProfilePictureUploadResponse>(
    `/student/${userId}/profile-picture`,
    {
      method: "POST",
      body,
    },
...
export function processOpportunityDescription(
  userId: number,
  description: string,
) {
  return request<OpportunityProcessResponse>(
...
export function submitInstitutionOpportunity(
  opportunity: OpportunitySubmission,
) {
  return request<OpportunitySubmissionResponse>(
